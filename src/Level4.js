...
export function Level4({ setPNo, apiData }) {
  ...
  const Banner = data && data['65'] ? data['65'].find((image) => image.key.replace(/\.(jpeg|jpg|png)$/, '') === 'Level Completion Badge') : null;
  const ProgressBar = data && data['82'] ? data['82'].find((image) => image.key.replace(/\.(jpeg|jpg|png)$/, '') === 'Progress Bar') : null;
